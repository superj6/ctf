...
(function() {
  ...
  TrustedUrlRequestParams.encodedSize = codec.kStructHeaderSize + 64;
  ...
  URLRequest.decode = function(decoder) {
    ...
    val.isOutermostMainFrame = (packed >> 3) & 1 ? true : false;
    val.upgradeIfInsecure = (packed >> 4) & 1 ? true : false;
    val.isRevalidating = (packed >> 5) & 1 ? true : false;
    val.isFetchLikeApi = (packed >> 6) & 1 ? true : false;
    val.isFavicon = (packed >> 7) & 1 ? true : false;
    packed = decoder.readUint8();
  ...
  URLRequest.encode = function(encoder, val) {
    ...
    packed |= (val.isOutermostMainFrame & 1) << 3
    packed |= (val.upgradeIfInsecure & 1) << 4
    packed |= (val.isRevalidating & 1) << 5
    packed |= (val.isFetchLikeApi & 1) << 6
    packed |= (val.isFavicon & 1) << 7
    encoder.writeUint8(packed);
    packed = 0;
